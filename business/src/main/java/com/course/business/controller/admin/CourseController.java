...
public class CourseController {
    ...
    public ResponseDto save(@RequestBody CourseDto courseDto) {
        // 保存校验
        ValidatorUtil.require(courseDto.getName(), "名称"); // 非空校验
        ValidatorUtil.length(courseDto.getName(), "名称", 1, 50); //长度校验
        ValidatorUtil.length(courseDto.getSummary(), "概述", 1, 2000); //长度校验
        ValidatorUtil.length(courseDto.getImage(), "封面", 1, 100); //长度校验
