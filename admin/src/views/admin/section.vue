...
<script>
  ...
  export default {
    ...
    data:function(){
      return {
        section:{},// section 变量用于绑定form表单的数据
        sections: []
      }
    },
    mounted:function () {
    ...
    methods:{
      add(){
        let _this = this;
        _this.section={};// 这个是保证在每一次新增的时候 不会带入上一次修改时候留下的数据  因为上次修改数据还保存在section中没有被清空
        // 此处不是一个方法
        $("#form-modal").modal("show")
      },
      edit(section){
        let _this = this;//1 点击要弹出模态框  2  模态打开以后要展示对应id的记录的数据
        // _this.section = section;// 此处修改其他的地方也跟着进行了改变
        _this.section = $.extend({},section);// 使用jquery的方法  复制一份给模态框展示需要 当用户取消的时候，就不会把值给数据库表格数据
        $("#form-modal").modal("show");
